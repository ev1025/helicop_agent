...
VLLM_PYTHON="${VLLM_PY:-$PY}"
CONFIG_TEMPLATE="${CONFIG_FILE:-$REPO/autorag/config.server.yaml}"
...
VLLM_TIMEOUT="${VLLM_TIMEOUT:-300}"      # 25분 (큰 모델 다운로드 대비)
...
start_vllm() {
    ...
    nohup env CUDA_VISIBLE_DEVICES="$VLLM_GPU" "$VLLM_PYTHON" -m vllm.entrypoints.openai.api_server \
        --model "$model" \
    ...
    while true; do
        ...
        # die check via vllm log error pattern (정상 startup ValueError 감지)
        if grep -qE "raise ValueError|Engine core initialization failed|RuntimeError" "$VLLM_LOG" 2>/dev/null; then
            log_summary "[vllm] error detected in log for $model after ${waited}s"
            return 1
        fi
...
for spec in "${MODEL_SPECS[@]}"; do
    ...
    # 5) 평가 (timeout 적용)
    rm -rf "$PROJECT_DIR" "$REPO"/autorag_chroma_*  # 매 모델마다 새 indexing
    eval_start=$(date +%s)
